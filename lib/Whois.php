...
Web Site:    http://www.phpeasycode.com/whois/
...
class Whois {
    ...
    public function domain_registrant($domain){
        $info = $this->domain_info($domain);
        $info_list = explode("\n" ,$info);
        $key = array_search('Registrant:', $info_list);
        if ($key===false){
            $key = array_search('No Match', $info_list);
            if ($key!==false){
                throw new Exception("Invalid Domain: $domain");
            }       
        }
        return trim($info_list[$key + 1]);
    }
    ...
    private function LookupDomain($domain){
            ...
            if($result=='') {
                //throw new Exception("Error: No results retrieved from $whoisserver server for $domain domain!");
            }else {
